...
def list_trucks(
    ...
    # Latest mixer temperature (°F) per truck, for the live truck card. One pass over
    # recent readings that carry a temp; first hit per truck is the newest.
    trows = s.exec(select(MixerReading)
...
def delete_unmatched_fuel(vehicle_no: str, k: str = Query(""), purge: int = Query(0),
    ...
    if k != "ab-vision-7f3a9c2e":
        raise HTTPException(404, "Not found")
    if purge:
        rows = s.exec(select(FuelTransaction)).all()
        n = len(rows)
        for ft in rows:
            s.delete(ft)
        s.commit()
